...
To restore the scratchable area to its initial, unscratched state, call `resetScratchingProgress(_:)`:

```swift

resetScratchingProgress(updateCoveragePercentage: true) // or false

```

-  **updateCoveragePercentage**: Pass `true` if you want the scratching coverage percentage (0) to be updated and reported via the `percentageScratched` callback.

---

### Revealing the Hidden Content

To instantly reveal the entire hidden image, call `showScratched(_:)`:

```swift

showScratched(updateCoveragePercentage: true) // or false

```

-  **updateCoveragePercentage**: Pass `true` if you want the scratching coverage percentage (100) to be updated and reported via the `percentageScratched` callback.